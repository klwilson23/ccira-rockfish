...
m3jTMB <- glmmTMB(counts~depth+depth:species+species+gear+as.factor(PU_4Km_ID)+offset(log(effort)),dispformula = ~gear,ziformula = ~species,data=new_df,family=nbinom2,control=glmmTMBControl(optCtrl=list(iter.max=5e6,eval.max=5e6)))
...
m3hTMB2 <- glmmTMB(counts~depth+I(depth^2)+species+gear+as.factor(PU_4Km_ID)+offset(log(effort)),dispformula = ~gear,ziformula = ~UpperOceanSR,data=new_df,family=nbinom2,control=glmmTMBControl(optCtrl=list(iter.max=5e6,eval.max=5e6)))
m3jTMB2 <- glmmTMB(counts~depth+depth:species+species+gear+as.factor(PU_1Km_ID)+offset(log(effort)),dispformula = ~gear,ziformula = ~species:gear,data=new_df,family=nbinom2,control=glmmTMBControl(optCtrl=list(iter.max=5e6,eval.max=5e6)))
...
m3cTMBzi18 <- glmmTMB(counts~depth+depth:species+depth:UpperOceanSR+I(depth^2)+species+gear+offset(log(effort)),dispformula = ~gear,ziformula=~species,data=new_df,family=nbinom2,control=glmmTMBControl(optCtrl=list(iter.max=5e4,eval.max=5e4)))
m3cTMBzi19 <- glmmTMB(counts~species+gear+offset(log(effort)),ziformula=~as.factor(PU_4Km_ID),data=new_df,family=truncated_nbinom2(),control=glmmTMBControl(optCtrl=list(iter.max=5e5,eval.max=5e5),profile=TRUE))
#m3cTMBzi20 <- glmmTMB(counts~depth+depth:species+I(depth^2)+species+gear+offset(log(effort)),ziformula=~as.factor(PU_4Km_ID),data=new_df,family=truncated_nbinom2(),control=glmmTMBControl(optCtrl=list(iter.max=5e5,eval.max=5e5),profile=TRUE))

#m3cTMBzi19 <- glmmTMB(counts~depth+depth:species+depth:UpperOceanSR+species+gear+offset(log(effort))+1|PU_4Km_ID,dispformula = ~gear,ziformula=~species,data=new_df,family=nbinom2,control=glmmTMBControl(optCtrl=list(iter.max=5e4,eval.max=5e4)))
